...
} from '@/lib/server/risk-rules'
import { AssessmentSchema } from '@/lib/server/assessments/generator'
import { LONGITUDINAL_CODES, type LongitudinalCode } from '@/lib/shared/questionnaires/registry'
...
export async function getClinicianInbox(
  ...
  const [
    ...
    supabase
      ...
      .in('user_id', userIds)
      .eq('status', 'scored')
      .in('questionnaire_definitions.code', LONGITUDINAL_CODES as unknown as string[])
